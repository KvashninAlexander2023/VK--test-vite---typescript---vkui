...
import type { CompareFilm, PoiskkinoDoc } from "../utilites/types";
...
export const $compareFilms = createStore<CompareFilm[]>([])
  .on(addToCompare, (state, film) => {
    ...
    if (state.length >= 2) {
      ...
      const oldestIndex = state.reduce((oldest, item, index) => (item.addedAt < state[oldest].addedAt ? index : oldest), 0);
...
export const $compareFilmsList = $compareFilms.map((films) => films.map(({ addedAt, ...film }) => film));
...
export const $isInCompare = (id: number) => $compareFilms.map((films) => films.some((f) => f.id === id));
